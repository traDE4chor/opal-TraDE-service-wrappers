...
public class ProcessManager {
    ...
    public ProcessManager(TSimulationIdentifier simulationID, ApiClient tradeApiClient) {
        ...
        _dataElement2FileName.put("opal_in", PREFIX_FORMAT + "opal.in");
        _dataElement2FileName.put("lattice", PREFIX_FORMAT + "opalbcc.dat");
        _dataElement2FileName.put("snapshots[]", PREFIX_FORMAT + "abcd" + SUFFIX_FORMAT + ".dat");
        _dataElement2FileName.put("saturation", PREFIX_FORMAT + "doad.dat");
        _dataElement2FileName.put("report", PREFIX_FORMAT + "report.dat");
        _dataElement2FileName.put("energy", "econf.dat");
        _dataElement2FileName.put("imdtransfer[]", PREFIX_FORMAT + "imdtransfer" + SUFFIX_FORMAT + ".chkpt");
    }

    public void prepareProcessExecution(int prefix, int numberOfSnapshots, TDataModelReference modelReference,
                                        TDataElementRef...
        ...
        _numberOfSnapshots = numberOfSnapshots;
    ...
    private void downloadInputs(TDataElementRef... dataElements) throws Exception {
        ...
        for (TDataElementRef elementRef : dataElements) {
            ...
            if (isCollection) {
                // TODO:
                for (int index = 1; index <= _numberOfSnapshots; index++) {
                    byte[] data = _tradeManager.pull(_modelReference.getNamespaceURI(), _modelReference.getLocalName
                            (), elementRef.getDataObjectName(), elementRef.getDataElementName(), index);
                    ...
                    if (data != null) {
                        ...
                        Path filePath = _opalDataFolder.resolve(String.format(fileNameTemplate, _prefix, index));
    ...
    private TResultDataList uploadResults(TDataElementRef... dataElements) {
        ...
        for (TDataElementRef elementRef : dataElements) {
            ...
            if (isCollection &&
                    elementRef.getDataElementName().equals("snapshots[]")) {
                for (int index = 1; index <= _numberOfSnapshots; index++) {
                    String fileNameTemplate = _dataElement2FileName.get(elementRef.getDataElementName());
                    ...
                    if (data != null) {
                        String dataValueLink = _tradeManager.push(_modelReference.getNamespaceURI(), _modelReference.getLocalName
                                (), elementRef.getDataObjectName(), elementRef.getDataElementName(), isCollection,
                                data, getContentType(fileNameTemplate), "opalMC-WebService-Wrapper#", index);
                ...
                if (data != null) {
                    String dataValueLink = _tradeManager.push(_modelReference.getNamespaceURI(), _modelReference
                            .getLocalName
                                    (), elementRef.getDataObjectName(), elementRef.getDataElementName(), isCollection, data, getContentType(fileNameTemplate),"opalMC-WebService-Wrapper#",null);
